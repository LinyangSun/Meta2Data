...
Amplicon_Common_MakeManifestFileForQiime2() {
    ...
    if [ "$sequence_type" = "single" ]; then
        python py_16s.py mk_manifest_SE --FilePath $temp_file_path$dataset_name"-file.txt"
    else
        python py_16s.py mk_manifest_PE --FilePath $temp_file_path$dataset_name"-file.txt"
    fi
}
...
Amplicon_Illumina_DenosingDada2() {
    ...
    if $need_compute; then
        ...
        trim_pos_result="$(python py_16s.py trim_pos_deblur --FilePath "$tsv_path")"
